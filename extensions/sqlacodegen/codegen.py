...
class ModelGenerator(Model):
    ...
    def __init__(self, table: Table, association_tables: List, inflect_engine, detect_joined):
    ...
    def add_imports(self, collector):
        super(ModelGenerator, self).add_imports(collector)
...
class CodeGenerator(object):
    ...
    def __init__(self, metadata, table_name, class_name=None, no_indexes=False, no_constraints=False, no_joined=False,
                 no_backrefs=False, ignore_cols=None, no_classes=False, no_comments=False, no_tables=False):
        ...
        for table in sorted(metadata.tables.values(), key=lambda t: (t.schema or '', t.name)):
            ...
            # Only generate classes when no_tables is set to True
            if no_tables:
                model = ModelGenerator(table, links[table.name], inflect_engine, not no_joined)
                classes[model.name] = model
            elif not table.primary_key or table.name in association_tables or no_classes:
